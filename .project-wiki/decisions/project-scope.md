...
- 来源：用户明确“mpp在我们这里专指这一套使用hashdata的系统”，生产集群日常称为 mpp。
- 本项目业务语境中的 MPP／mpp 是该生产 HashData 系统的专名；讨论该系统专属表时，
  `mpp_` 表示这一系统范围，不表示所有采用 MPP 架构的数据库共用同一契约。
- 面向项目成员的说明可称 MPP，首次需要说明产品时写作“MPP（HashData）”；
  产品名称、源端版本和既有 `hashdata-csv/1` 等技术标识保留其准确含义。
- 用户同日补充“我们有两个跑批系统，分别是luban和baichuan”；记录 `luban` 和
