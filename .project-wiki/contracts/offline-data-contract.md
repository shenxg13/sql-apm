...
物理结构已有[初版映射](../../docs/design/postgresql-storage.md)，记录 text ID、numeric、
微秒时间、约束和应用责任；该实现不代替本契约或业务运行时校验器。

## Open Questions

内容身份生产算法、统计计算容差、完整 SQL 解析及配对算法、业务锁与事务、
序列化及运行时校验由后续实施任务落实；物理类型与表／索引见上方映射。
