...
并检查引用表及被引用表上的外键内部触发器均处于默认 O（origin/local）启用模式。
...
- 外键内部触发器处于 D（禁用）、R（仅 replica）或 A（always）均视为结构漂移；
  A 虽可在普通连接中执行，但仍不同于本版声明的默认模式。all／schema／check／upgrade
  共用此检查，失败诊断含 `foreign_key_trigger_overrides`，不会自动启用触发器。
...
| foreign_key_trigger_overrides | 外键内部触发器模式异常；先由实例维护者核对变更原因、异常时间段及受影响数据，确认后按既有授权恢复默认模式再重试。项目入口不提升账号权限或自动修改触发器。 |
| 库内 DDL 中断／失败 | 本阶段全部回滚，角色／库不回滚；修正原因后重跑。 |
| 同版本完整重跑 | 保留业务数据、账号密码及版本首次应用时间，返回 OK。 |

外键触发器重新启用只恢复后续语句的约束执行，不证明异常期间已经写入的数据合法。
本入口检查系统目录，不扫描或修复历史业务行；若有异常期间写入，应另行核查并处理受影响数据。
...
以及外键两侧内部触发器的 D／R／A 模式漂移、拒绝后状态保留及恢复后的真实外键拒绝。
