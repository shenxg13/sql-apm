...
[物理结构](../design/postgresql-storage.md)。不安装实例、不修改 pg_hba.conf 或现有实例配置。
...
结构版本和 schema.sql 的 SHA-256 也须一致。不能直接绕开入口运行 schema.sql。
...
是一笔事务。后续版本演进以本目录 SQL 和 schema_version 为入口，显式提供迁移及
验证；不能修改已部署的 1.0.0 DDL 后冒充兼容重跑。未来迁移平台不在本次范围内。
